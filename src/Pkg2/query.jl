...
function check_partial_updates(reqs::Requires,
    ...
    for p in upkgs
        ...
        if isfixed
            msg *= "      The package is fixed. You can try using `Pkg2.free(\"$p\")` to update it."
        ...
        elseif !isempty(blocking_parents)
            ...
            msg *= "      To install the latest version, you could try doing a full update with `Pkg2.update()`."
        end
        info(msg)
    end
end
