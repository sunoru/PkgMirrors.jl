...
import ...Pkg2: metadata_url, META_BRANCH, PkgError
...
function cd(f::Function, args...; kws...)
    ...
    if !isdir(metadata_dir)
        !haskey(ENV,"JULIA_PKGDIR") ? init() :
            throw(PkgError("Package metadata directory $metadata_dir doesn't exist; run Pkg2.init() to initialize it."))
    end
...
function init(meta::AbstractString=metadata_url(), branch::AbstractString=META_BRANCH)
    dir = path()
    ...
    try
        ...
        Base.cd(temp_dir) do
            ...
            LibGit2.with(LibGit2.clone(meta, "METADATA", branch = branch)) do metadata_repo
